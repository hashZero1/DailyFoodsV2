    ...
    <section className="mt-5">
      <div className="flex flex-wrap justify-center">
        {bulkRecipe?.map((dt) => (
          ...
          //   </div>
          // </motion.div>
          <div class="relative flex flex-col m-6 bg-white shadow-sm border border-slate-200 rounded-lg w-full md:w-[40%] lg:w-96">
            <button
              onClick={() => {
                addItemToCart(dt);
                notify();
            ...
            <div class="p-4">
              ...
              <h6 class="mb-2 text-slate-700 text-xl font-bold">{dt.title}</h6>
              <p
                class="text-gray-500 leading-normal font-md"
                dangerouslySetInnerHTML={{
                  __html: DOMPurify.sanitize(dt.summary.slice(0, 120) + "...."),
                }}
              />
            </div>
            ...
            <div class="flex items-center justify-between p-4">
              <div class="flex items-center">
                <button className="px-6 py-1 lg:px-4 lg:py-1 bg-gray-900 text-white font-semibold hover:bg-gray-800 hover:text-white transition-all rounded-lg">
                  <DetailsPageComponent recipe={dt} />
                </button>
              </div>
            </div>
